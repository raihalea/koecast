...
クライアントの接続先 (DGX の Tailscale MagicDNS 名)・ホットキー・用語辞書は、
リポジトリ内の `client/config.toml.example` をコピーして使う:

```sh
mkdir -p "$HOME/Library/Application Support/koecast"
cp client/config.toml.example "$HOME/Library/Application Support/koecast/config.toml"
# 開いて server_url を自分の DGX Tailscale 名で書き換える
$EDITOR "$HOME/Library/Application Support/koecast/config.toml"
```

実体の `config.toml` は **git 管理外**（`.gitignore` 済み）。テンプレートだけ
リポジトリに置く運用。

設定キー:

- `server_url`: gateway の WebSocket URL。`ws://<dgx-tailscale-name>:8000/v1/dictation`
- `hotkey`: プッシュトゥトークのキー (`global-hotkey` 形式、6-3-c で使用)
- `glossary`: セッション単位の用語ヒント (6-2-c の LLM 整形プロンプトに合流)

環境変数 `KOECAST_CLIENT_CONFIG=<path>` を指定するとその path を優先して読む
（テスト時や複数プロファイル切り替えに使う想定）。
...
- `client/src-tauri/tauri.conf.json` の `build.devUrl` は `http://localhost:5173`。
- 別プロセスが 5173 を使っていないか確認 (`lsof -i :5173`)。
- `npm run dev` (vite のみ) を別ターミナルで先に起動して、`http://localhost:5173`
  がブラウザで開けるか単体確認。

### マイクから音が録れない / 権限プロンプトが出ない (段階6-3-c 以降)

- `client/src-tauri/Info.plist` の `NSMicrophoneUsageDescription` が消えていないか確認。
  この key が無いと macOS は **権限プロンプトすら出さず無音 PCM を返す** (Riva に何も
  届かないので `session_end segments=0` が連発する)。
- `Info.plist` は Tauri CLI が dev ビルドでも自動マージしてくれる。バイナリ更新が
  伝わるよう、編集後は `npm run tauri dev` を一度止めて再起動する。
- 一度プロンプトで「許可しない」を選んでしまった場合: `システム設定 → プライバシー
  とセキュリティ → マイク` で `koecast-client` を許可に変える。

### ホットキーを押しても無反応 (段階6-3-c 以降)

- macOS の **システムショートカットと衝突**している可能性。代表的な衝突:
  - `Cmd+Space` → Spotlight
  - `Cmd+Option+Space` → Finder の Spotlight 検索ウィンドウ
- 衝突が少ない `Ctrl+Shift+Space` を `config.toml` の `hotkey` に設定し直して
  アプリを再起動する。
- ホットキー登録は `RegisterEventHotKey` (Carbon API) 経由なので、`入力監視` 権限は
  不要。捕捉できない=システム側で先に消費されている、と切り分ける。

### overlay 等の追加ウィンドウで Tauri API (listen/invoke) が動かない

- `src-tauri/capabilities/default.json` の `windows: [...]` に **その window の
  label が含まれていない**と、その webview からの `@tauri-apps/api/*` 呼び出しは
  permission denied で例外になる (UI は描画されるが reactive update が止まる)。
- **新規 window を `tauri.conf.json` に追加したら、必ず capability の `windows`
  にも label を追加する**こと。段階6-3-d で overlay 追加時にこれでハマった。
  例: `"windows": ["main", "overlay"]`

### テキスト注入が動かない / 権限プロンプトが出ない (段階6-3-e 以降)

- 注入は `cargo run` 生成バイナリ
  `client/target/debug/koecast-client` に対する macOS の **アクセシビリティ権限**
  が要る。自動プロンプトが出ないことが多いので、`システム設定 → プライバシーと
  セキュリティ → アクセシビリティ` の `+` ボタンで上記パスを `Cmd+Shift+G` で
  入力して追加 → トグル ON。
- **TCC はバイナリパス + チェックサム単位で記録**される。`cargo run` で再ビルド
  しても同じパスなら設定が引き継がれるが、**段階6-4-b で `.app` バンドル化
  する/別 path にコピーするとパスが変わり、権限の再付与が必要**になる。配布版
  リリース時もユーザーに同じ手順を案内する想定。
- 段階6-3-e の Mac 実装は `osascript` 経由 (`tell application "System Events"
  to keystroke "v" using command down`) で Cmd+V を送る。今のところ
  アクセシビリティ権限だけで動いているが、macOS バージョン更新等で別途
  **「オートメーション」枠 (System Events を制御するアプリの許可)** を求められる
  可能性がある。その場合も同じプライバシーとセキュリティ配下に出るので許可する。
