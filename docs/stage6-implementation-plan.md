...
- **qwen36-mtp が `Up (unhealthy)` 状態である** (2026-05-25 時点で観測)。
  - 段階6-2 自走実行中、`docker compose ps` で qwen36-mtp に `(unhealthy)` ラベルが
    付いていた。一方、LLM 整形 (`scenario_streaming_ja_with_wav_and_llm`) は
    成功しており、`/v1/chat/completions` の疎通自体は問題ない。
  - 実害の有無は人間が別途調査予定。Claude Code 側からは対応しない。
  - **段階6-3 のクライアント結合確認で LLM 起因の不具合
    (formatted が空 / fallback が頻発 / timeout) が出たらこれを疑う**。
    `docker inspect qwen36-mtp` の `Health.Log` を見ると healthcheck の失敗内容が
    分かる。
