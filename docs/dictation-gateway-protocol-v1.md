...
- `context`: そのセッション限定で追加する Word Boosting 語。常用辞書はサーバ側が保持。
...
- 機械可読スキーマは `protocol/schema/client-to-server.schema.json` と
  ...
  通すまで実装に進まない。
- **未確定（実装前に確定が必要）**:
  - Riva の ARM64 / GB10 対応、および日本語ストリーミングモデルの有無（要 NGC 検証）。
    不可の場合はバッチ Whisper にフォールバックし、本プロトコルの `partial` を
    省略した簡易版に縮退する。
  - 句読点付与を Riva 側で行う前提（`final` は句読点付き）。LLM 側はフィラー除去と
    用語補正に専念する。
