...
- server (Pydantic v2): `server/src/dictation_gateway/protocol.py` + 契約テスト
  (22件、`just test-shared` で通過)。
- client (serde): `client/src-tauri/src/protocol.rs` + 契約テスト
  (7件、negative ケース込み、`just test-shared` で通過)。
...
2. 分離後も `just test-shared` の client 側が通ることを確認
3. `client/src-tauri/Cargo.toml` に `tauri` / `tauri-build` を復活。
   `devenv.nix` への Linux 用 OS 依存追記は**行わない**: Tauri ビルド・実行は
   Mac/Windows 実機で行う方針 (`docs/stage6-implementation-plan.md` §1.5)。
