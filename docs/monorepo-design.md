...
モノレポは Python・Rust・Node の3トーチェーンを抱える。これを Raiha の既存の
devenv/Nix ワークフローで一括管理する。

- **`devenv.nix`**: Python(uv)・Rust(cargo)・Node・`just`・Tauri のシステム依存
  （WebKitGTK 等）を1ファイルで宣言。`devenv shell` で全員（＝Raiha）が同じ環境。
...
- 型生成（3.3）を採用するか、手書き＋ゴールデンテスト（3.4）だけで回すか。
  まずは後者で始め、drift が頻発したら前者を足すのが現実的。
- フロントエンドフレームワークの確定（Svelte を仮置き）。
