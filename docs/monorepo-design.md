...
│   │   ├── riva_bridge.py  # Riva ASR NIM への gRPC ストリーミング中継 / 後処理
│   │   ├── llm.py          # LLM 整形クライアント
...
- **`riva_bridge.py` の追加責務**（`docs/stage6-readiness.md` §2.2 / §2.3 / §3.1 の
  実機検証結果に基づく）:
  - **CJK 文字間の空白除去**: Riva ASR NIM は ja 出力に 1 字ごとに半角空白を入れて
    返すため、partial / final 両方に対して regex で除去する。
  - **`segment_id` の採番**: Riva の `StreamingRecognitionResult` には `segment_id`
    相当のフィールドが無いため、接続開始時 0、`is_final=True` を観測したら
    インクリメント、を `riva_bridge.py` 側で行う。
  - **Word Boosting (`speech_contexts`) は使わない**: ja に対して機能しないことが
    実機確認済み。仕様 §4.1 の `context` フィールドは `llm.py` 側の用語辞書として
    使う。
- **設定**: `config.py` が環境変数 + 設定ファイルを読む。Riva アドレス（既定
  `localhost:50051`）、LLM エンドポイント、用語辞書、システムプロンプト、
  バインドアドレス（tailnet インターフェース）。
- **デプロイ**: `server/deploy/docker-compose.yml` で DGX 上に起動。前回作成した
  vLLM/Whisper の Compose と同じ流儀。**Riva 本体は別 Compose で
  `nvcr.io/nim/nvidia/parakeet-1-1b-rnnt-multilingual:1.4.0` を起動**し
  （`NIM_TAGS_SELECTOR="mode=str"`、bind mount 先は host で事前 mkdir 必須）、
  ゲートウェイは gRPC 50051 で接続する（同居推奨）。Riva Quick Start ARM64 は
  L4T/Jetson 専用で DGX Spark では使えないため採用しない。詳細は
  `docs/stage6-readiness.md` §2.0 / §2.1。
...
- **`justfile`**: 言語横断のタスクを集約。
  ...
  - `just test-shared` — server (pytest) と共有 protocol クレート (cargo test
    `-p koecast-protocol`) の契約テストを通す。Tauri 本体は含まない
    (§10/§11、`docs/stage6-implementation-plan.md` §1.5 参照)
  - `just lint` / `just fmt`
...
- **first partial レイテンシの真値**: ファイル投入では 2.84 s 観測。マイクからの
  リアルタイム入力で「発話開始 → first partial 表示」を段階6 でクライアント実装後に
  実機計測。許容できなければ Riva の `endpointing_config` チューニング、
  `mode=str-thr` 比較、または overlay 表示戦略で調整。
  詳細は `docs/stage6-readiness.md` §2.4 と `docs/stage6-implementation-plan.md` §4。

---

## 11. 確定事項（段階1〜5 と Riva 実機検証フェーズで決まったもの）
...
- **プロトコル契約テストは server 側 (Pydantic + jsonschema) を先行整備済み**。
  client (serde) は段階5 で追加済み。
- **STT バックエンド**: Riva ASR NIM の
  `nvcr.io/nim/nvidia/parakeet-1-1b-rnnt-multilingual:1.4.0`
  (`NIM_TAGS_SELECTOR="mode=str"`)。DGX Spark / Blackwell 公式サポート、
  ja-JP 対応、ストリーミング対応。`riva_bridge.py` は `nvidia-riva-client` で
  gRPC 50051 を叩く。タグ `:1.4.0` 固定の理由とキャッシュ bind mount の事前 mkdir
  要件は `docs/stage6-readiness.md` §2.0 を参照。
- **ja 後処理は server 側で吸収**: CJK 文字間空白除去・`segment_id` 採番・
  Word Boosting 不使用は `riva_bridge.py` の責務（§4 / `docs/stage6-readiness.md` §3.1）。
- **段階6-3 以降の開発の場の切り分け**: server (Python) と protocol (Rust serde) は
  DGX Spark の `devenv shell` (Linux) で扱い、Tauri クライアント本体は
  **Raiha の手元の Mac/Windows 実機**でビルド・実行・動作確認する。
  `devenv.nix` に Linux 用 Tauri OS 依存 (WebKitGTK / libsoup / dbus 等) は
  追記しない。§10 で長らく保留してきた「実機ビルド検証時の `devenv.nix` 追記」の
  「実機」は Mac/Windows と確定。詳細は `docs/stage6-implementation-plan.md` §1.5。
